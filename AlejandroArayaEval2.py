...
def read_student_agg():
    ...
    students_index = 0
    students_agg_message = "Desea agregar las notas de un estudiante? ? [y/n]: "
    ...
    while not y_n_break_flag:
        if students_index != 0:
            students_agg_message = "Desea agregar las notas de un estudiante adicional? [y/n]: "
        ...
        if y_n.lower() == "y":
            ...
            lowest_grade = get_highest_grade(grade, lowest_grade)
            if is_even(grade):
                even_grade_count += 1
            else:
                odd_grade_count += 1
